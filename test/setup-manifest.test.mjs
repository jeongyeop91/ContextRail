...
const valid = {
  ...
  throughline: {
    ...
    artifact: {
      ...
      url: 'https://github.com/jeongyeop91/ContextRail/releases/download/v0.3.0-rc.4/throughline-0.10.3-codex.2.tgz',
      sha256: 'a'.repeat(64),
    },
  },
};
...
test('accepts an immutable setup manifest and selects only Throughline', () => {
  const result = validateSetupManifest(valid, { expectedVersion: '0.3.0-rc.4' });
...
test('rejects mutable assets, malformed digests, unknown platforms, version drift, and unknown keys', () => {
  ...
  input.extra = true;
  const result = validateSetupManifest(input, { expectedVersion: '0.3.0-rc.4' });
...
test('loads the checked-in release-candidate selection with matching Throughline provenance', async () => {
  const result = await loadSetupManifest({ root: process.cwd(), fs: nodeFilesystem, expectedVersion: '0.3.0-rc.4' });
  assert.equal(result.ok, true);
