...
test('assembles byte-identical registry and GitHub assets with a detached integrity envelope', async () => {
  ...
  const result = await assembleReleaseAssets({
    ...
    packageMetadata: { name: 'contextrail', version: '0.3.0-rc.6' },
    fs: nodeFilesystem,
  });
  const names = (await nodeFilesystem.list(scope.output)).sort();
  assert.deepEqual(names, [
    'SHA256SUMS.txt',
    'contextrail-0.3.0-rc.6.tgz',
    'contextrail.tgz',
    'release-manifest.json',
    'throughline-0.10.3-codex.2.tgz',
  ]);
  const versionedBytes = await readFile(join(scope.output, 'contextrail-0.3.0-rc.6.tgz'));
  ...
  assert.equal(result.releaseVersion, '0.3.0-rc.6');
});
