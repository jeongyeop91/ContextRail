...
```text
npm install --global https://github.com/jeongyeop91/ContextRail/releases/download/v0.3.0-rc.11/contextrail-0.3.0-rc.11.tgz
contextrail setup
```

Verify the CLI with `contextrail --version`; the expected candidate version is `0.3.0-rc.11`.
...
- `0.3.0-rc.11` is the npm `next` release candidate; `latest` remains gated on the Windows live pilot.
